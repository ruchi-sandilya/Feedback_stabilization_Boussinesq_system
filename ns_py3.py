...
class HeatSource(Expression):
   def eval(self, value, x):
      ...
      return value
#-----------------------------------------------------------------------------
class SourcePerturbation(Expression):
   def eval(self, value, x):
...
class NSProblem():
   ...
   def get_indices(self):
      ...
      freeinds = np.setdiff1d(list(range(N)),bcinds,assume_unique=True).astype(np.int32)
   ...
   def get_indices_lagrange(self):
      ...
      for b1 in self.bc1:
         ...
         bcinds1.extend(list(bcdict1.keys()))

      # Control dirichlet dofs
      bcinds2 = []
      for b2 in self.bc2:
         bcdict2 = b2.get_boundary_values()
         bcinds2.extend(list(bcdict2.keys()))
      ...
      freeinds = np.setdiff1d(list(range(N)),bcinds1,assume_unique=True).astype(np.int32)
